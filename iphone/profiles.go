...
func (ip *iPhone) handleProfileMessage(senderUUID string, data []byte) {
	...
	ip.meshView.MarkProfileReceived(deviceID, 1) // TODO: Use actual version
...
func (ip *iPhone) handleProfileRequest(senderUUID string, req *proto.ProfileRequestMessage) {
	prefix := fmt.Sprintf("%s iOS", ip.hardwareUUID[:8])
...
func (ip *iPhone) sendProfileToDevice(targetUUID string) error {
	...
	if err := ip.connManager.SendToDevice(targetUUID, phone.AuraProfileCharUUID, data); err != nil {
		return fmt.Errorf("failed to send profile: %w", err)
	}

	logger.Debug(prefix, "📤 Sent profile to %s (v%d)", targetUUID[:8], ip.localProfile.ProfileVersion)
	return nil
}
